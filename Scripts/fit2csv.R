  ...
  else 
  {
   ...
   fln3 <- paste(folder,paste(name,"c.csv",sep = ""), sep = "/")
    if (cls=="kfsst"){
     ...
     write.table(data.frame(name,fit$date,fit$most.prob.track,fit$var.most.prob.track[,c(1,4)],fit$nominal.track,fit$SST[,c(1,3)]), append=T, col.names=F, sep=",", na="", file=fln1)
   ...
   if (cls=="kftrack"){
     ...
     write.table(data.frame(name,fit$date,fit$most.prob.track,fit$var.most.prob.track[,c(1,4)],fit$nominal.track), append=T, col.names=F, sep=",", na="", file=fln1)
     # Add labels, rearrange, fill gaps
