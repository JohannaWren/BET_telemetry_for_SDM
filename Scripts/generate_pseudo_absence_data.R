...
# we first need to ensure all our GPE3 .csvs are in the same folder, when they are run below code and that will join them together.

##regex - Final data/Perfect_data
files_to_read = list.files(
  path = here('Data/GPE3files_ALL/'),   # directory to search within
...
bsh.tracks = out_df %>%
  dplyr::select(lon,lat,date,tagID, Observation.Type)  ## JLKW added Observation type because it's needed later
bsh.tracks <- na.omit(bsh.tracks)      ### JLKW removed NA's from data frame
...
ggplot()+
  ...
  geom_sf(data=poly,fill=NA,color="black")+
  scale_color_manual("PresAbs",values=c("1"="blue","0"="red"))+
  coord_sf(xlim = c(140, 260), ylim = c(miny,maxy), expand=FALSE) +
